...
"-------------------------------------------\r\n"
" Welcome to Rongan-net Device command shell\r\n"
"-------------------------------------------\r\n"
"\r\n";
...
format_parser_read_word(struct format_parser_state *state)
{
	...
	else if (strcmp(cmd, "YYYY-MM-DD") == 0)
		token->terminal = TERMINAL_DATE;
	else if (strcmp(cmd, "hh:mm:ss") == 0)
...
static enum match_type  cmd_date_match(const char *str) {
	const char *cp;
	char year[5] = { 0 };
	char month[3] = { 0 };
	char day[3] = { 0 };
	...
	if (pcats[0]) {
		if (pcats[0] - cp != 4)
			return no_match;
		memcpy(year, cp, pcats[0] - cp);
		y = atoi(year);
		if (y < 1970)
			return no_match;
	}
	if (pcats[1]) {
		if (pcats[1] - pcats[0] != 2)
			return no_match;
		if (str - pcats[1] > 2)
			return no_match;
		memcpy(month, pcats[0], pcats[1] - pcats[0]);
		m = atoi(month);
		if (m > 12)
			return no_match;
		memcpy(day, pcats[1], str - pcats[1]);
		d = atoi(day);
		if (d > 31)
			return no_match;
	}
...
static enum match_type  cmd_time_match(const char *src) {
	const char *cp;
	char buf[3] = { 0 };
	int colons = 0;
	const char *pcolons[2] = { NULL, NULL };
	int h = 0;
	int m = 0;
	int s = 0;
	...
	if (pcolons[0]) {
		...
		memcpy(buf, cp, pcolons[0] - cp);
		h = atoi(buf);
		if (h > 23)
			return no_match;
	}
	if (pcolons[1]) {
		if (pcolons[1] - pcolons[0] != 2) {
			return no_match;
		}
		if (src - pcolons[1] > 2)
			return no_match;

		memcpy(buf, pcolons[0], pcolons[1] - pcolons[0]);
		m = atoi(buf);
		if (m >= 60)
			return no_match;
		s = atoi(pcolons[1]);
	...
	const char *word)
{
	...
	case TERMINAL_IPV4_PREFIX:
		...
		break;
	case TERMINAL_DATE:
		match_type = cmd_date_match(word);
		if ((filter == FILTER_RELAXED && match_type != no_match)
			|| (filter == FILTER_STRICT && match_type == exact_match))
			return date_match;
		break;
	case TERMINAL_TIME:
		match_type = cmd_time_match(word);
		if ((filter == FILTER_RELAXED && match_type != no_match)
			|| (filter == FILTER_STRICT && match_type == exact_match))
			return time_match;
		break;
			...
			for (j = 0; j < vector_active(match_vector); j++)
				if ((cmd_token = vector_slot(match_vector, j)) != NULL)
				{
					...
					case extend_match:
						...
						break;
					case date_match:
						if (cmd_token->terminal == TERMINAL_DATE)
							match++;
						break;
					case time_match:
						if (cmd_token->terminal == TERMINAL_TIME)
							match++;
						break;
					case no_match:
					default:
...
cmd_entry_function_desc(const char *src, struct cmd_token *token)
{
	...
	case TERMINAL_IPV4_PREFIX:
		if (cmd_ipv4_prefix_match(src))
			...
			return NULL;
	case TERMINAL_DATE:
		if (cmd_date_match(src))
			return dst;
		else
			return NULL;
	case TERMINAL_TIME:
		if (cmd_time_match(src))
			return dst;
		else
			return NULL;
...
int config_from_file(struct vty *vty, FILE *fp, unsigned int *line_num)
{
	...
	*line_num = 0;
	...
	while (!feof(fp))
	{
		...
		if(fgets(vty->buf, vty->max, fp) == NULL)
		{
			break;
		}
		if (vty->buf[0] == 0x00)
...
static reset_config_cb _reset_config_cb = NULL;

void reset_config_cb_register(reset_config_cb cb)
{
	_reset_config_cb = cb;
}

DEFUN(reset_configuration,
	reset_configuration_cmd,
	"reset configuration",
	RESET_STR
	"customer configuration\n")
{
	char *config_file;
	char *config_file_priv;
	...
	config_file = host.config;
	config_file_priv =
		XMALLOC(MTYPE_TMP, strlen(config_file) + strlen(CONF_PRIV_EXT) + 1);
	strcpy(config_file_priv, config_file);
	strcat(config_file_priv, CONF_PRIV_EXT);
	unlink(config_file);
	link(config_file_priv, config_file);
	sync();

	XFREE(MTYPE_TMP, config_file_priv);
...
ALIAS(reset_configuration,
	reset_configuration_o_cmd,
	"outer reset configuration",
	"outer machine\n"
	RESET_STR
	"customer configuration\n");
...
install_default_basic(enum node_type node)
{
	...
	install_ignore_log_cmd(&config_exit_cmd);
	install_ignore_log_cmd(&config_quit_cmd);
	install_ignore_log_cmd(&config_end_cmd);
	install_ignore_log_cmd(&config_list_cmd);
}
...
cmd_init(int terminal)
{
	...
	if (terminal)
	{
		...
		install_element(CONFIG_NODE, &reset_configuration_cmd);
		install_element(CONFIG_NODE, &reset_configuration_o_cmd);

	}
	install_element(CONFIG_NODE, &show_commandtree_cmd);

	cmd_common_init(terminal);
...
struct hash *_ignore_cmd_hash;
void ignore_log_cmd_init() {
	_ignore_cmd_hash = hash_create(cmd_hash_key, cmd_hash_cmp);
}

void install_ignore_log_cmd(struct cmd_element *cmd) {
	hash_get(_ignore_cmd_hash, cmd, hash_alloc_intern);
}

int is_ignore_log_cmd(struct cmd_element *cmd) {
	return hash_lookup(_ignore_cmd_hash, cmd) ? 1 : 0;
}
