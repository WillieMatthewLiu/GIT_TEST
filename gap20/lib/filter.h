...
struct filter_zebra
{
	...
	/* add by lxd, for port ctl in iptables */
	int port;
	char proto[16];
...
extern void access_list_add_hook(void(*func)(struct access_list *, struct filter *filter));
extern void access_list_delete_hook(void(*func)(struct access_list *, struct filter *filter));
