	...
	if_lookup_by_name_len_vrf(const char *name, size_t namelen, vrf_id_t vrf_id)
{
	...
	for (ALL_LIST_ELEMENTS_RO(vrf_iflist(vrf_id), node, ifp))
	{
		if (!memcmp(name, ifp->name, namelen) && (ifp->name[namelen] == '\0'))
			return ifp;
		if (!memcmp(name, ifp->alias, namelen) && (ifp->alias[namelen] == '\0'))
			return ifp;
	}
	return NULL;
}
...
#define IFF_OUT_LOG(X,STR) \
  if (flag & (X)) \
	...
	IFF_OUT_LOG(IFF_MASTER, "MASTER");
	IFF_OUT_LOG(IFF_SLAVE, "SLAVE");
...
ALIAS(interface,
	interface_outer_vrf_cmd,
	"interface outer IFNAME " VRF_CMD_STR,
	"Select an interface to configure\n"
	"indicate outer side\n"
	"Interface's name\n");
