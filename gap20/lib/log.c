...
static struct vty_chain log_operation_chain;
...
ZLOG_FUNC(zlog_emerg, LOG_EMERG)

ZLOG_FUNC(zlog_critical, LOG_CRIT)

ZLOG_FUNC(zlog_alert, LOG_ALERT)
		...
		int syslog_flags, int syslog_facility)
{
	...
	vty_chain_register(&log_operation_chain);
...
#define INSERT_OP_LOG(ip, user, op, accesstype, content, ret) \
do{\
	char sql[2048]={0};\
    snprintf(sql, 2048, SQL_INSERT_OP_FMT, BOARDTYPE_STR, ip, user, op, accesstype, content, ret);\
    insert_into(sql); \
}while(0)

static const char *get_op_by_cmd(char *cmd) 
{
	...
	if (strncmp(cmd, "end", strlen("end")) == 0
		|| strncmp(cmd, "configure", strlen("configure")) == 0
		|| strncmp(cmd, "inner", strlen("inner")) == 0
		|| strncmp(cmd, "outer", strlen("outer")) == 0) 
	{
		return NULL;
	}
...
static int log_operation_cb(struct vty *vty, int ret)
{
	return 0;
	...
	if (vty->username == NULL || op == NULL)
	{
		return 0;
	}		

	INSERT_OP_LOG(vty->login_ipaddr, vty->username, op, vty->access, p, ret);
