	...
	thread_master_create()
{
	...
	pthread_mutex_init(&rv->thread_lock, &attr);
	pthread_mutexattr_destroy(&attr);
#else
	pthread_mutex_init(&rv->thread_lock, NULL);
#endif
	return rv;
}
...
thread_cancel(struct thread *thread)
{
	...
	pthread_mutex_lock(&thread->master->thread_lock);
	...
	case THREAD_BACKGROUND:
		...
		break;
	default:
		pthread_mutex_unlock(&thread->master->thread_lock);
		return;
		break;
	}
