...
#define SCGetThreadIdLong(...) ({ \
   pid_t tmpthid; \
   tmpthid = pthread_self();/*syscall(SYS_gettid);*/ \
   u_long tid = (u_long)tmpthid; \
   tid; \
})
#endif /* OS FREEBSD */
...
/* If no signal, waiting for 1s */
#define SCCondTimedWait1s_dbg(cond, mut) ({ \
	struct timespec stTime; \
	clock_gettime(CLOCK_MONOTONIC, &stTime); \
	stTime.tv_sec += 1; \
    stTime.tv_nsec = 0; \
    int ret = pthread_cond_timedwait(cond, mut, &stTime); \
    switch (ret) { \
        case EINVAL: \
        printf("The value specified by attr is invalid (or a SCCondT not initialized!)\n"); \
        printf("%16s(%s:%d): (thread:%"PRIuMAX") failed SCCondTimedWait1s %p ret %" PRId32 "\n", __FUNCTION__, __FILE__, __LINE__, (uintmax_t)pthread_self(), mut, retu); \
        break; \
    } \
    ret; \
})
#define SCCondTimedWait1s SCCondTimedWait1s_dbg

#else

#define SCCondWait(cond, mut) pthread_cond_wait(cond, mut)
/* If no signal, waiting for 1s */
#define SCCondTimedWait1s(cond, mut) ({ \
	struct timespec stTime; \
	clock_gettime(CLOCK_MONOTONIC, &stTime); \
	stTime.tv_sec += 1; \
    stTime.tv_nsec = 0; \
