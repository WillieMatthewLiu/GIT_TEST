...
void vty_chain_run(struct vty *vty, int ret);
...
static struct vty_master _vty_master = {};

static struct vty_chain_root _vty_chain_root = {};
...
static int vty_log_out(struct vty *vty, const char *level, const char *proto_str,
...
vty_prompt(struct vty *vty)
{
	...
	if (vty->type == VTY_TERM)
	{
		...
		if (vty->node == INTERFACE_NODE)
		{
			...
			vty_out(vty, cmd_prompt(vty->node), hostname, iface->name);
		}
		else
...
vty_create(int vty_sock, union sockunion *su)
{
	...
	for (int i = 0; i < HOOKS; i++)
		if (_vty_master._vty_create[i])
			_vty_master._vty_create[i](vty);
	return vty;
}
...
static void vty_serv_un(const char* path)
{
	...
	if (unlink(path) < 0)
	{
		...
		if (nError != ENOENT)
		{
			zlog_err("Cannot delete %s file: %s", path, safe_strerror(errno));
			return;
		}
	}
	...
	int len = serv.sun_len = SUN_LEN(&serv);
#else
	int len = sizeof(serv.sun_family) + strlen(serv.sun_path);
#endif /* HAVE_STRUCT_SOCKADDR_UN_SUN_LEN */

	int ret = bind(sock, (struct sockaddr*) &serv, len);
	if (ret < 0)
	{
...
int vty_serv_sock(const char* addr, unsigned short port, const char* path)
{
	int fd;
...
#ifdef HAVE_IPV6
		fd = vty_serv_sock_addrinfo(addr, port);
#else /* ! HAVE_IPV6 */
		fd = vty_serv_sock_family(addr, port, AF_INET);
#endif /* HAVE_IPV6 */
	}
...
void vty_close(struct vty *vty)
{
	...
	for (i = 0; i < HOOKS; i++)
		if (_vty_master._vty_close[i])
			_vty_master._vty_close[i](vty);
	...
	for (i = 0; i < vector_active(vtyvec); i++)
		if ((v = vector_slot(vtyvec, i)) != NULL) {
			if (v->username) {
				struct tm * timeinfo;
				memset(buf, 0, 128);
				timeinfo = localtime(&v->login_time);
				strftime(buf, 128, "%F %T", timeinfo);
				vty_out(vty, "%svty[%d] connected from %s[%s] user %s login time %s%s",
					v->config ? "*" : " ",
					i, v->access,
					v->access[0] == 'c' ? "ttyS0" : v->login_ipaddr,
					v->username,
					buf, VTY_NEWLINE);
			}
			else {
...
void vty_serv_cancel(int fd)
{
	int i;
	struct thread *vty_serv_thread;

	for (i = 0; i < vector_active(Vvty_serv_thread); i++)
		if (((vty_serv_thread = vector_slot(Vvty_serv_thread, i)) != NULL) && (i == fd))
		{
			thread_cancel(vty_serv_thread);
			vector_slot(Vvty_serv_thread, i) = NULL;
			close(i);
...
vty_init(struct thread_master *master_thread)
{
	...
	pthread_mutex_init(&_vty_chain_root.lock, NULL);
...
void vty_add_hook(int type, int(*func)(struct vty *))
{
	int i;
	switch (type) {
	case VTY_CREATE_HOOK:
		for (i = 0; i < HOOKS; i++)
		{
			if (!_vty_master._vty_create[i])
			{
				_vty_master._vty_create[i] = func;
				break;
			}
		}
		break;
...
#define VTY_CHAIN_LOCK() \
    pthread_mutex_lock(&_vty_chain_root.lock)

#define VTY_CHAIN_UNLOCK() \
    pthread_mutex_unlock(&_vty_chain_root.lock)

void vty_chain_register(struct vty_chain *chain)
{
	struct vty_chain **root = &_vty_chain_root.root;
...
void vty_chain_unregister(struct vty_chain *chain)
{
	struct vty_chain **root = &_vty_chain_root.root;
	struct vty_chain *prev = NULL;
...
void vty_chain_run(struct vty *vty, int ret)
{
	struct vty_chain *chain = _vty_chain_root.root;

	VTY_CHAIN_LOCK();
	while (chain)
	{
		if (chain->func(vty, ret))
			zlog_err("run chain %p fail\n", chain->func);
		chain = chain->next;
	}
