...
#define VTY_ADPT_BUF_LEN 0x100000
struct vty_adapter {
	char *ip;
	int port;
	int fd;
	struct timeval timer_val;
	struct vty_adapter *next;
	char *buf;
	char data[0];
};

#define   VTY_CREATE_HOOK 1
#define   VTY_CLOSE_HOOK 2

struct vty_master {
#define HOOKS 16
	int(*_vty_create[HOOKS])(struct vty *);
	int(*_vty_close[HOOKS])(struct vty *);
};

struct vty_chain {
	int(*func)(struct vty* vty, int ret);
	struct vty_chain *next;
};

struct vty_chain_root {
	struct vty_chain *root;
	pthread_mutex_t lock;
};

/* Integrated configuration file. */
#define INTEGRATE_DEFAULT_CONFIG "app-all.conf"

/* Small macro to determine newline is newline only or linefeed needed. */
#define VTY_NEWLINE  ((vty->type == VTY_TERM) ? "\r\n" : "\n")

/* Default time out value */
#define VTY_TIMEOUT_DEFAULT 3600

/* Vty read buffer size. */
#define VTY_READ_BUFSIZ (512*4)
...
extern void vty_serv_cancel(int fd);
extern struct vty *vty_new(void);
extern struct vty *vty_stdio(void(*atclose)(void));
extern int vty_out(struct vty *, const char *, ...) PRINTF_ATTRIBUTE(2, 3);
extern void vty_read_config(char *, char *);
extern void vty_time_print(struct vty *, int);
extern int vty_serv_sock(const char *, unsigned short, const char *);
...
extern void vty_add_hook(int type, int(*func)(struct vty *));
