...
zebra_interface_if_set_value(struct stream *s, struct interface *ifp)
{
	...
	stream_get(ifp->alias, s, INTERFACE_NAMSIZ);
	...
	zebra_interface_address_read(int type, struct stream *s, vrf_id_t vrf_id)
{
	...
	{
		label = XMALLOC(MTYPE_CONNECTED_LABEL, STREAM_READABLE(s) + 1);
		label[STREAM_READABLE(s)] = '\0';
		stream_get(label, s, STREAM_READABLE(s));
	}
