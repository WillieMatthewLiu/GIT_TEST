...
vtysh_config_parse_line(const char *line)
{
	...
	default:
		...
		else if (strncmp(line, "local-user", 10) == 0) {
			config = config_get(DPI_FW_USER_NODE, line);
		}
		else if (strncmp(line, "app", 3) == 0)
			config = config_get(APP_NODE, line);
		else
		{
...
vtysh_config_dump(FILE *fp)
{
	...
	for (i = 0; i < vector_active(configvec); i++)
		if ((master = vector_slot(configvec, i)) != NULL)
		{
			...
			{
				if ((config->flags & FLAG_NO_PRINT_NAME) == 0) {
					fprintf(fp, "%s\n", config->name);
					fflush(fp);
				}
