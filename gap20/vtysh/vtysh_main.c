...
static void log_it(const char *line)
{
	...
	fprintf(logfile, "%s User:%s Process:%d Command:%s\n", tod, user, getppid(), line);
}

static void set_username_access(void)
{
	int len;
	FILE *fp_read = NULL;
	char *p = NULL, *q = NULL;
	...
	while ((pentry = getutent()) != NULL)
	{
		if (getpid() == pentry->ut_pid)
		{
			strcpy(username, pentry->ut_user);
			username[strlen(pentry->ut_user)] = '\0';
			strcpy(access, pentry->ut_line);
			access[strlen(pentry->ut_line)] = '\0';
			break;
		}
	}
	...
	{
		buf[0] = '\0';
		get_result_by_system(buf, sizeof(buf), "perl -ple 's,\\00, ,g' /proc/%d/cmdline", getppid());
		if (p = strchr(buf, '@')) {
			q = strchr(buf, ':');
			memcpy(username, q + 1, p - q - 1);

			q = p + 1;
			while (*q != ' ' && *q != '\0')
				q++;
			memcpy(line, p + 1, q - p - 1);

			setutent();
			while ((pentry = getutent()) != NULL)
			{
				if (strncmp(pentry->ut_line, line, strlen(line)) == 0)
				{
					strncpy(ip, pentry->ut_host, sizeof(ip));
					break;
				}
			}
		}
	...
	//printf("access: %s, username: %s, ip: %s\n", access, username, ip);

	if (access[0] == 't') {
		len = snprintf(buf, sizeof(buf), "login name %s access console", username);
	}
	else {
		len = snprintf(buf, sizeof(buf), "login name %s access ssh ip %s port 22", username, ip);
	}
	vtysh_execute(buf);
...
main(int argc, char **argv, char **env)
{
	...
	{
		fprintf(stderr, "WARNING: failed to connect to any daemons.%s", VTY_NEWLINE);
		fprintf(stderr, "\tPlease check the system.%s", VTY_NEWLINE);
	}
	/* If eval mode. */
	if (cmd)
	{
	...
	snprintf(history_file, sizeof(history_file), "%s/.history_rongan", getenv("HOME"));
