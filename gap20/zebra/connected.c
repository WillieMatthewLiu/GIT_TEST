...
struct connected *
connected_check (struct interface *ifp, struct prefix *p, const char *label)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (ifp->connected, node, ifc)){
    if (prefix_same (ifc->address, p))
      return ifc;
    if (label && ifc->label){
        if(strchr(label, ':') && (strcmp(label, ifc->label)==0))
            return ifc;
        if(!strchr(label, ':') && (strcmp(label, strchr(ifc->label, ':')+1)== 0))
            return ifc;
    }
  }
...
connected_update(struct interface *ifp, struct connected *ifc)
{
  ...
  if ((current = connected_check (ifp, (struct prefix *) ifc->address, ifc->label)))
    {
		       ...
		       u_char prefixlen, struct in_addr *broad)
{
  ...
  ifc = connected_check (ifp, (struct prefix *) &p, NULL);
  if (! ifc)
    return;
