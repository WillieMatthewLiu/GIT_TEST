...
$ignore{'"dpi (p0p1|p2p3)"'} = "ignore";
$ignore{'"terminal monitor {number <1-100000>|filter WORD}"'} = "ignore";
$ignore{'"local-user WORD type (administrator|general)"'} = "ignore";
$ignore{'"app"'} = "ignore";
$ignore{'"ha"'} = "ignore";
$ignore{'"inner"'} = "ignore";
$ignore{'"outer"'} = "ignore";
$ignore{'"arbiter"'} = "ignore";
...
foreach (@ARGV) {
    ...
    #open (FH, "gcc -E -DHAVE_CONFIG_H -DVTYSH_EXTRACT_PL -DHAVE_IPV6 -I.. -I./ -I./.. -I../lib -I../utils  $file |cat|");
    open (FH, "cat $file |");
    local $/; undef $/;
    $line = <FH>;
    close (FH);

    @defun = ($line =~ /(?:DEFUN|DEFUN_HIDDEN)\s*\((.+?)\);?\s?\s?\n/sg);
