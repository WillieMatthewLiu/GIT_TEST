...
static void if_bonding_dump_vty(struct vty *vty, struct interface *ifp)
{
    struct ifbond bonding;
    struct ifslave slave;

    if(if_bonding_query(ifp, &bonding) < 0)
        return;

    vty_out (vty, "  bonding mode   :%s%s", bond_mode_tbl[bonding.bond_mode], VTY_NEWLINE);

    
    vty_out (vty, "  bonding slaves : ");
    if(bonding.num_slaves)
    {
        char slaves[128]={0};
        get_result_by_system(slaves, 128, "cat /sys/class/net/%s/bonding/slaves", ifp->name);
        
        vty_out (vty, "%s ", slaves);
    }
...
if_dump_vty (struct vty *vty, struct interface *ifp)
{
  ...
  if(zebra_if->is_outer)
  {
     if(ifp->alias[0])
        vty_out (vty, "Interface outer %s(alias %s) is ", &ifp->name[1], ifp->alias);
      else
        vty_out (vty, "Interface %s is ", &ifp->name[1]);
  }
  if(ifp->alias[0])
    vty_out (vty, "Interface %s(alias %s) is ", ifp->name, ifp->alias);
  else
  ...
  if (ifp->flags&IFF_MASTER)
  {
      if_bonding_dump_vty(vty,  ifp);
  }
  if (zebra_if->gw != NULL)
  {
		char buf[PREFIX_STRLEN];
		prefix2str(zebra_if->gw, buf, sizeof(buf));
		char *token = strchr(buf, '/');
		if (token)
			*token = '\0';
		vty_out (vty, "  gateway : %s  isdefault:  %d%s", buf, zebra_if->isdefault, VTY_NEWLINE);
  }
}

#define INTERFACE_SHOW_ADPT(element, side) \
    do{\
        if(RUN_AS_INNER() && strstr(element->string, side)){\
            if(vty->usr_data){\
              return vty_adapter_run(vty,(struct vty_adapter *) vty->usr_data);\
            }else{\
              vty_out(vty, "don't connect backgroud server.%s", VTY_NEWLINE);\
              return CMD_SUCCESS;\
            }\
        }\
  }while(0)

#define INTERFACE_CONF_ADPT(zif) \
do{\
    if(zif && zif->is_outer && RUN_AS_INNER() ){\
        if(vty->usr_data){\
          return vty_adapter_run(vty,(struct vty_adapter *) vty->usr_data);\
        }else{\
          vty_out(vty, "no outer board online!!%s", VTY_NEWLINE );return CMD_WARNING;\
        }\
    }\
}while(0)
...
DEFUN_NOSH (zebra_outer_interface,
	    zebra_outer_interface_cmd,
	    "interface outer IFNAME",
	    "Select an interface to configure\n"
	    "Indicate it is Outer side\n"
	    "Interface's name\n")
{
  int ret;
  struct interface * ifp;
  char *newif[1];
  if(board_type == BOARDTYPE_IN )
    {
    struct zebra_if *if_data;
    newif[0] = XMALLOC(MTYPE_VTY, INTERFACE_NAMSIZ + 1);
    snprintf(newif[0], INTERFACE_NAMSIZ, "o%s", argv[0]);
    if ((ret = interface_cmd.func (self, vty, 1, newif)) != CMD_SUCCESS){
        XFREE(MTYPE_VTY, newif[0]);
      return ret;
        }
...
ALIAS (show_interface, show_outer_interface_cmd,
       "show outer interface",
       SHOW_STR
       OUTER_STR
       "Interface status and configuration\n")
       ...
       VRF_ALL_CMD_HELP_STR)
{
  ...
  INTERFACE_SHOW_ADPT(self, "outer");
...
ALIAS (show_interface_vrf_all, show_outer_interface_vrf_all_cmd,
       "show outer interface " VRF_ALL_CMD_STR,
       SHOW_STR
       OUTER_STR
       "Interface status and configuration\n"
       VRF_ALL_CMD_HELP_STR)
...
ALIAS (show_interface_name, show_outer_interface_name_cmd,
       "show outer interface IFNAME",
       SHOW_STR
       OUTER_STR
       "Interface status and configuration\n"
       "Inteface name\n")
...
ALIAS (show_interface_name_vrf_all, show_outer_interface_name_vrf_all_cmd,
       "show outer interface IFNAME " VRF_ALL_CMD_STR,
       SHOW_STR
       OUTER_STR
       "Interface status and configuration\n"
       "Inteface name\n"
...
ALIAS(show_interface_desc,
       show_interface_desc2_cmd,
       "show outer interface description",
       SHOW_STR
       OUTER_STR
       "Interface status and configuration\n"
       "Interface description\n");
       ...
       "Set multicast flag to interface\n")
{
  ...
  zif->multicast = IF_ZEBRA_MULTICAST_ON;
       ...
       "Unset multicast flag to interface\n")
{
  ...
  zif->multicast = IF_ZEBRA_MULTICAST_OFF;
		    ...
		    const char *label)
{
  ...
  ifc = connected_check (ifp, (struct prefix *) &cp, label);
  if (! ifc)
    {
      ifc = connected_new ();
      ifc->ifp = ifp;
	  ...
	  *p = cp;
      ...
      if (label)
        ...
        if(strchr(label,':') == NULL){
            ifc->label = XMALLOC(MTYPE_CONNECTED_LABEL,strlen(label)+INTERFACE_NAMSIZ + 2);
            snprintf(ifc->label, strlen(label)+INTERFACE_NAMSIZ + 2, "%s:%s", ifp->name, label);
        }
        else
      ...
      listnode_add (ifp->connected, ifc);
        ...
        vty_out(vty, "ip address %s or label %s is exist.%s", addr_str, label, VTY_NEWLINE);
  ...
  if (! CHECK_FLAG (ifc->conf, ZEBRA_IFC_QUEUED)
      ...
      && !(zif && zif->shutdown == IF_ZEBRA_SHUTDOWN_ON))
    {
		      ...
		      const char *label)
{
  ...
  ifc = connected_check (ifp, (struct prefix *) &cp, NULL);
  if (! ifc)
    {
		      ...
		      const char *label, int secondary)
{
  ...
  /* This addres is add to outer */
  if(board_type == BOARDTYPE_IN &&zif&& zif->is_outer)
    return CMD_SUCCESS;
			...
			const char *label, int secondry)
{
  ...
  /* This addres is delete from outer */
    if(board_type == BOARDTYPE_IN &&zif&& zif->is_outer)
      {
      listnode_delete (ifp->connected, ifc);
      connected_free (ifc);
...
DEFUN(bonding_set_mode,
        bonding_set_mode_cmd,
        "mode <0-6>",
        "bonding mode\n"
        "model enum\n")
{
    ...
    if (!(ifp->flags & IFF_MASTER))
    {
        vty_out(vty, "only master port support set mode.%s", VTY_NEWLINE);
        return CMD_SUCCESS;
    }
    if(ifp->flags & IFF_UP)
    {
        vty_out(vty, "interface is UP, shutdown it first.!!");
        return CMD_SUCCESS;
    }
    if(!zif->bond)
    {
        zif->bond = XMALLOC(MTYPE_IF, sizeof(struct ifbond));
        if_bonding_query(ifp, zif->bond);
    }

    get_result_by_system(NULL, 0, "echo %s > /sys/class/net/%s/bonding/mode", argv[0], ifp->name);
...
DEFUN(bonding_add_slaves,
    bonding_add_slaves_cmd,
    "member IFNAME",
    "bonfing member add\n"
    "which interface will add to bonding\n")
{
    struct listnode *node;
    struct interface *slave, *slave_r;
    
    struct interface *master = vty->index;

    struct zebra_if *zif= master->info;

    INTERFACE_CONF_ADPT(zif);

    if(!(master->flags & IFF_MASTER))
    {
        vty_out(vty, "only master port support add member.%s", VTY_NEWLINE);
        return CMD_ERR_NOTHING_TODO;
    }
    ...
    {
        zif->bond = XMALLOC(MTYPE_IF, sizeof(struct ifbond));
        if_bonding_query(master, zif->bond);
    }
    if(!zif->slaves)
    {
        int i = 0;
        zif->slaves = list_new();
        zif->slaves->cmp = if_cmp_func;
        for(i = 0; i < 16; i++){
            struct ifslave s;
            s.slave_id = i;
            if(if_bonding_slave_query(master, &s) <0)
                continue;
            slave = if_lookup_by_name(s.slave_name);
            if(slave)
                listnode_add_sort(zif->slaves, slave);
        }
    }
    
    slave = if_lookup_by_name(argv[0]);
    if(!slave)
    {
        vty_out(vty, "interface %s don't exist.%s", argv[0], VTY_NEWLINE);
        return CMD_ERR_NOTHING_TODO;
    }
    for(ALL_LIST_ELEMENTS_RO(zif->slaves,node,slave_r))
    {
        if(if_cmp_func(slave, slave_r) ==0)
        {
            return CMD_ERR_NOTHING_TODO;
        }
    }

    if(if_bonding_add_mem(master, slave) < 0)
    {
        vty_out(vty, "add member fail.%s", VTY_NEWLINE);
        return CMD_ERR_NOTHING_TODO;
    }
    listnode_add_sort(zif->slaves, slave);
...
DEFUN(bonding_remove_slaves,
    bonding_remove_slaves_cmd,
    "no member IFNAME",
    NO_STR
    "bonfing member add\n"
    "which interface will add to bonding\n")
{
    ...
    if(if_bonding_remove_mem(master, slave) < 0)
    {
        vty_out(vty, "remove member fail.%s", VTY_NEWLINE);
        return CMD_ERR_NOTHING_TODO;
    }
    listnode_delete(zif->slaves, slave);

    return CMD_SUCCESS;
}

DEFUN(linkmode_change,
    linkmode_change_cmd,
    "(full|half) duplex",
    "Full duplex\n"
    "Half duplex\n"
...
DEFUN(no_linkmode_change,
    no_linkmode_change_cmd,
    "no (full|half) duplex",
    NO_STR
    "Full duplex\n"
    "Half duplex\n"
...
DEFUN (default_gateway,
       default_gateway_cmd,
       "gateway A.B.C.D default (enable|disable)",
       "gateway command\n"
       "IP address (e.g. 10.0.0.1)\n"
       "Set default gateway\n"
       "default gateway is enable\n"
       "default gateway is disable\n"
       )
{
	struct zebra_if *zif;
	struct interface *ifp = vty->index;
	...
	if (0 == strcmp(argv[1], "enable"))
	{
		...
		struct zebra_if *zif2;
		vrf_iter_t iter;

		for (iter = vrf_first (); iter != VRF_ITER_INVALID; iter = vrf_next (iter))
		for (ALL_LIST_ELEMENTS_RO (vrf_iter2iflist (iter), node, ifp2))
		{
			zif2 = ifp2->info;
			if ((zif2->gw != NULL)&&(zif2->isdefault == 1))
			{
				 char buf[PREFIX_STRLEN] = "";
		 		 prefix2str(zif2->gw, buf, sizeof(buf));
				 char *token = strchr(buf, '/');
				if (token)
					*token = '\0';
				sprintf(cmd, "route del default gw %s dev %s", buf, ifp2->name);
				cmd_system_novty(cmd);
				zif2->isdefault = 0;
			}
		}
		sprintf(cmd, "route add default gw %s dev %s", argv[0], ifp->name);
		cmd_system_novty(cmd);
		zif->isdefault = 1;
	}
	else
	{
		if ((zif->gw != NULL)&&(zif->isdefault == 1))
		{
			char buf[PREFIX_STRLEN] = "";
		 	prefix2str(zif->gw, buf, sizeof(buf));
			char *token = strchr(buf, '/');
			if (token)
				*token = '\0';
			sprintf(cmd, "route del default gw %s dev %s", buf, ifp->name);
			cmd_system_novty(cmd);
		}
		zif->isdefault = 0;
	}
	...
	*p = cp;
	...
	if (zif->gw == NULL)
	{
		zif->gw = (struct prefix *)p;
	}
	else
	{
		prefix_ipv4_free((struct prefix_ipv4 *)(zif->gw));
		zif->gw = (struct prefix *)p;
	}
	ifp->gw = zif->gw;
...
static int if_config_bonding_write(struct vty *vty, struct interface *ifp)
{
    int count = 0;
    struct listnode *node;
    struct interface *slave;
    ...
    {
        vty_out(vty, " member %s%s", slave->name, VTY_NEWLINE);
        count++;
    }
  ...
  for (ALL_LIST_ELEMENTS_RO (vrf_iter2iflist (iter), node, ifp))
    {
      ...
      if(zif && zif->is_outer){
	    ...
	    if (ifp->vrf_id == VRF_DEFAULT)
        	vty_out (vty, "interface outer %s%s", &ifp->name[1],
	      	  VTY_NEWLINE);
	    else
		   vty_out (vty, "interface outer %s vrf %u%s", &ifp->name[1], ifp->vrf_id,
	      	  VTY_NEWLINE);
#else
        continue;
#endif
	...
	vty_out(vty, " bandwidth %u%s", ifp->bandwidth, VTY_NEWLINE); 
      ...
      if (zif)
	{
	  if (zif->multicast != IF_ZEBRA_MULTICAST_UNSPEC)
	    vty_out (vty, " %smulticast%s",
		     zif->multicast == IF_ZEBRA_MULTICAST_ON ? "" : "no ",
		     VTY_NEWLINE);
	}
      ...
      if(ifp->flags&IFF_MASTER)
        if_config_bonding_write(vty, ifp);
	  ...
	  if (zif->gw != NULL)
	  {
		...
		if (token)
		  ...
		  if (zif->isdefault == 1)
		  {
			  vty_out (vty, " gateway %s default enable%s", buf, VTY_NEWLINE);
		  }
		  else
		  {
			  vty_out (vty, " gateway %s default disable%s", buf, VTY_NEWLINE);
		  }
	  }
...
zebra_if_init (void)
{
  ...
  install_element (VIEW_NODE, &show_outer_interface_cmd);
  install_element (VIEW_NODE, &show_interface_vrf_all_cmd);
  install_element (VIEW_NODE, &show_outer_interface_vrf_all_cmd);
  install_element (VIEW_NODE, &show_interface_name_cmd);
  install_element (VIEW_NODE, &show_outer_interface_name_cmd);
  install_element (VIEW_NODE, &show_interface_name_vrf_cmd);
  install_element (VIEW_NODE, &show_interface_name_vrf_all_cmd);
  install_element (VIEW_NODE, &show_outer_interface_name_vrf_all_cmd);
  install_element (CONFIG_NODE, &zebra_interface_cmd);
  install_element (CONFIG_NODE, &zebra_outer_interface_cmd);
  ...
  install_element (INTERFACE_NODE, &bonding_set_mode_cmd);
  install_element (INTERFACE_NODE, &bonding_add_slaves_cmd);
  install_element (INTERFACE_NODE, &bonding_remove_slaves_cmd);
  install_element (INTERFACE_NODE, &default_gateway_cmd);
