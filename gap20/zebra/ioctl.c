...
int if_bonding_query(struct interface *ifp, struct ifbond *bonding)
{
    int ret;
    struct ifreq ifreq;

    ifreq_set_name (&ifreq, ifp);
    ifreq.ifr_ifru.ifru_data = (void *)bonding;

    ret = if_ioctl (SIOCBONDINFOQUERY, (caddr_t) &ifreq);
    if (ret < 0) 
    {
        zlog_err("if_ioctl(SIOCBONDENSLAVE) failed: %s", safe_strerror(errno));
        return ret;
    }
    return ret;
}

int if_bonding_slave_query(struct interface *ifp, struct ifslave *slave)
{
    int ret;
    struct ifreq ifreq;

    ifreq_set_name (&ifreq, ifp);
    ifreq.ifr_ifru.ifru_data = (void *)slave;

    ret = if_ioctl (SIOCBONDSLAVEINFOQUERY, (caddr_t) &ifreq);
    if (ret < 0) 
    {
        zlog_err("if_ioctl(SIOCBONDENSLAVE) failed: %s", safe_strerror(errno));
        return ret;
...
int if_bonding_add_mem(struct interface *ifp, struct interface *slave_ifp)
{
    int ret;
    struct ifreq ifreq;

    ifreq_set_name (&ifreq, ifp);
    strncpy(ifreq.ifr_ifru.ifru_slave, slave_ifp->name, IFNAMSIZ);

    ret = if_ioctl (SIOCBONDENSLAVE, (caddr_t) &ifreq);
    if (ret < 0) 
    {
        zlog_err("if_ioctl(SIOCBONDENSLAVE) failed: %s", safe_strerror(errno));
        return ret;
...
int if_bonding_remove_mem(struct interface *ifp, struct interface *slave_ifp)
{
    int ret;
    struct ifreq ifreq;
    ...
    ret = if_ioctl (SIOCBONDRELEASE, (caddr_t) &ifreq);
    if (ret < 0) 
    {
        zlog_err("if_ioctl(SIOCBONDRELEASE) failed: %s", safe_strerror(errno));
        return ret;
    }
